...
public class StoreFacade {
	...
	public boolean addInventory(Product newItem) throws SQLException,ClassNotFoundException
	{	int UPC=newItem.getUPC();
		
		PreparedStatement stmt = Scon.prepareStatement("SELECT * FROM inventoryDB where UPC =?");
		stmt.setInt(1, UPC);
		ResultSet rs = stmt.executeQuery();
	...
	public boolean changeInventory(Product newItem)  throws SQLException
	{
	int UPC=newItem.getUPC();
	PreparedStatement stmt = Scon.prepareStatement("REPLACE * FROM inventoryDB where UPC =?");
	stmt.setInt(1, UPC);
	ResultSet rs = stmt.executeQuery();
