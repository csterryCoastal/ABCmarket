...
public Item(String name,int UPC,double price,String PU)
{
	this.name=name;
	this.UPC=UPC;
	this.price=price;
...
public double purchasePrice(double quantity)
{
	...
	{
		if(priceUnit.toLowerCase().equals('e'))// If price unit is per item
			return Math.ceil(quantity)*price*salePercent;// If quantity is decimal then round up for per each price
			else
			return quantity*price*salePercent;
